...
var Cin_Header_Ename = 'MyPhonetic';
var Cin_Header_Cname = '我的注音輸入法';
var Cin_File_Path = 'CnsPhonetic.cin';
var ErrorLog = '';
var FileBuf = []; //https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array
var FileBuf_Len = 0;
var CnsUniTable = [];
var CnsPhonTable = [];

var Cns_Unicode_FileList = [
	'data/CNS2UNICODE_Unicode_2.txt',
	'data/CNS2UNICODE_Unicode_15.txt',
	'data/CNS2UNICODE_Unicode_BMP.txt'
];

var Cns_Phonetic_FileList = [
	'data/CNS_phonetic.txt',
	'data/Other_phonetic.txt'
];
...
function createCin(file, data)
{
	...
	try {
	    let ok = GLib.file_set_contents(file, data);
		if (!ok) {
			print('Error createCin: ');
			System.exit(1);
	    }
	} catch (e) {
	    print('Error createCin: ', e.message);
		System.exit(1);
	}
}

function loadCnsUnicode()
{
	FileBuf = []; //reset
	...
	for (var file of Cns_Unicode_FileList) {
		loadEachCnsUnicode(file);
	}
...
function loadEachCnsUnicode(file)
{
	//print(file);
	...
	try {
	    let [ok, text] = GLib.file_get_contents(file);
	    if (!ok) {
			print('Error loadEachCnsUnicode: ');
			System.exit(1);
	    }
		FileBuf.push(text.toString());
	} catch (e) {
	    print('Error loadEachCnsUnicode: ', e.message);
		System.exit(1);
	}
...
function loadCnsPhonetic()
{
	...
	//https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/for...of
	//https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Array_comprehensions
	for (var file of Cns_Phonetic_FileList) {
		loadEachCnsPhonetic(file);
	}
...
function loadEachCnsPhonetic(file)
{
	//console.log(file);
	...
	try {
	    let [ok, text] = GLib.file_get_contents(file);
	    if (!ok) {
			print('Error loadEachCnsPhonetic: ');
			System.exit(1);
	    }
		FileBuf.push(text.toString());
	} catch (e) {
	    print('Error loadEachCnsPhonetic: ', e.message);
		System.exit(1);
	}
...
function SortTables()
{
	...
	createCin(Cin_File_Path,out_str);
...
function getHeadStr()
{
	...
	str="\
%gen_inp\n\
%ename "+Cin_Header_Ename+"\n\
%cname "+Cin_Header_Cname+"\n\
%selkey 0123456789\n\
%keyname begin\n\
, ㄝ\n\
- ㄦ\n\
...
{
	//var l,u,h,v,key; // key 這個變數，重複定義了
	var l,u,h,v;
	l=0;
...
//http://locutus.io/php/strings/chr/
function toUTF16(uc)
{
	if(uc>0xFFFF)
	{
